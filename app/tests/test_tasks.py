...
@pytest.mark.anyio
async def test_generate_cute_creture_api_success(mock_httpx_client):
...
@pytest.mark.anyio
async def test_generate_cute_creture_api_server_error(mock_httpx_client):
    mock_httpx_client.post.return_value = httpx.Response(
        status_code=500, content="", request=httpx.Request("POST", "//")
    )
...
@pytest.mark.anyio
async def test_generate_cute_creture_api_json_error(mock_httpx_client):
    mock_httpx_client.post.return_value = httpx.Response(
